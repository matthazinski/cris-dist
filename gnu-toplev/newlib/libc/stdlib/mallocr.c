...
/* We must make sure these are the real definitions for newlib.  */
...
/* We also need to define strong aliases for _Newlib_malloc_r_actual
   etc. to the real ones, overriding the weak definitions in mallocw.c et
   al.  See comment in mallocw.c.  */

#ifdef DEFINE_MALLOC
_PTR _Newlib_malloc_r_actual (struct _reent *, size_t n)
     __attribute__ ((alias ("_malloc_r")));
#endif

#ifdef DEFINE_FREE
void _Newlib_free_r_actual (struct _reent *, _PTR aptr)
     __attribute__ ((alias ("_free_r")));
#endif

#ifdef DEFINE_REALLOC
_PTR _Newlib_realloc_r_actual (_PTR ap, size_t size)
     __attribute__ ((alias ("_realloc_r")));
#endif

#ifdef DEFINE_CALLOC
_PTR _Newlib_calloc_r_actual (size_t n, size_t size)
     __attribute__ ((alias ("_calloc_r")));
#endif
...
#endif
{
  ...
  /* Fail if sbrk failed or if a foreign sbrk call killed our space,
     unless sbrk is declared to return random chunks.  */
  if (brk == (char*)(MORECORE_FAILURE)
#ifndef NONSTANDARD_SBRK
