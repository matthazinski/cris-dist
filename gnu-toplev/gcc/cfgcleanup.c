  ...
  if (!optimize_size)
    {
      ...
      /* We don't want to replace code with a branch to the identical
	 code.  However, if we can replace the *whole* block with an
	 unconditional branch, we can eliminate that branch later.  */
      if (newpos1 != src1->head)
	return false;

      /* Don't replace a block that is on a fallthrough edge with a
         branch.  */
      for (src1_pred_e = src1->pred;
	   src1_pred_e;
	   src1_pred_e = src1_pred_e->pred_next)
	if ((src1_pred_e->flags & EDGE_FALLTHRU) != 0)
	  return false;
    }
      ...
      do
	{
	  ...
	  for (i = 0; i < n_basic_blocks;)
		...
		changed_here = true;
	      ...
	      /* After crossjumping, there might be single forwarder
		 jumps scattered all over, with "complex" edges.  Merge
		 with successor block.  */
	      if ((mode & CLEANUP_CROSSJUMP) != 0
		  && FORWARDER_BLOCK_P (b)
		  && simplejump_p (b->end)
		  && LABEL_NUSES (block_label (b)) != 0)
		{
		  edge tmp_edge;
		  ...
		  if (tmp_edge == NULL)
		    {
		      rtx moved_label = block_label (b);
		      rtx to_label = block_label (b->succ->dest);
		      rtx after = NEXT_INSN (to_label);
		      edge e;

		      /* We insert the code-label from b (there's only
			 one in a bb) into the head of the successor
			 (after the block label, splitting that block),
			 forward the edges and delete the block rather
			 than trying to merge the block with the
			 successor.  If we'd use
			 merge_blocks_move_predecessor_nojumps, we'd
			 still have to forward edges that used to lead
			 into the successor ourselves, and the block
			 label of the successor would be deleted (which
			 isn't really right, and would likely break
			 something). */

		      /* Unfortunately split_block doesn't search for
			 and handle existing block notes, so split the
			 block after the existing note.  */
		      if (!NOTE_INSN_BASIC_BLOCK_P (after)
			  || b->head != moved_label)
			abort ();
		      b->head = NEXT_INSN (moved_label);
		      reorder_insns_nobb (moved_label, moved_label, after);
		      if (b->succ->dest->end == after)
			b->succ->dest->end = moved_label;

		      /* The split_block call also handles the change of
			 bb for moved_label.  */
		      e = split_block (b->succ->dest, after);

		      while (b->pred)
			redirect_edge_succ_nodup (b->pred, e->dest);
