...
#define CRIS_CC1_SUBTARGET_SPEC \
 "%{!march=*:%{!cpu=*:-march=v32}}"
#define CRIS_SUBTARGET_DEFAULT_ARCH TARGET_MASK_AVOID_GOTPLT
#else
#define CRIS_CC1_SUBTARGET_SPEC \
 "%{!march=*:%{!cpu=*:-march=v10}}"
...
#if TARGET_CPU_DEFAULT == 32
#define CRIS_SUBTARGET_VERSION " - crisv32-axis-linux-gnu"
#else
...
#define CRIS_LINK_SUBTARGET_SPEC \
 "-mcrislinux\
  %{L*:-rpath-link %* }\
  %{!nostdlib:-rpath-link include/..%s\
    -rpath-link ../sys-include/asm/../../lib%s\
    -rpath-link ../include/asm/../../lib%s}\
  %{shared} %{static}\
