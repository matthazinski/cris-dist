...
/* BEGIN AXIS LOCAL
   ...
   Hopefully this isn't necessary with gcc-3.3.x and up; I didn't see
   the exact same bug there.  There are no REG_INC notes automatically
   attached to splits, yet flow expects them.  (Maybe other places too.)
   See gcc.c-torture/execute/20040915-Axis1.c for CRISv32; the DI
   postincrement loses the REG_INC note and one of the loads is deleted
   without the address being updated for the insn using the
   postincremented address.  Rather than adding them on each split in
   the md, we do it automatically in this wrapper function.  Instead of
   the function with this name being emitted in insn-recog.c, it is
   here, and the function in insn-recog.c is instead split_insns_1.  */

extern rtx split_insns_1 PARAMS ((rtx, rtx));

/* Helper function: argument to for_each_rtx.  */

static int add_incdec_note (rtx *xp, void *data)
{
  rtx *note = data;
  rtx x = *xp;

  if (x != NULL_RTX
      /* The 'a' stands for the auto_inc-class.
	 (Fixed to be an enum in a later release.)  */
      && GET_RTX_CLASS (GET_CODE (x)) == 'a'
      && REG_P (XEXP (x, 0)))
    *note = alloc_EXPR_LIST (REG_INC, XEXP (x, 0), *note);

  return 0;
}

/* Wrapper function adding REG_INC notes.  */

rtx
split_insns (rtx x0, rtx insn)
{
  rtx result = split_insns_1 (x0, insn);

#ifdef AUTO_INC_DEC
  if (result == NULL_RTX)
    return NULL_RTX;

  if (GET_CODE (result) == SEQUENCE)
    {
      int i;
      for (i = 0; i < XVECLEN (result, 0); i++)
	for_each_rtx (&XVECEXP (result, 0, i), add_incdec_note,
		      &REG_NOTES (XVECEXP (result, 0, i)));
    }
  else
    for_each_rtx (&result, add_incdec_note, &REG_NOTES (result));
#endif
