     ...
     int flags;
{
  ...
  if (optimize && (flags & PROP_SCAN_DEAD_CODE))
    {
      if (GET_CODE (reg) == REG || GET_CODE (reg) == MEM)
     ...
     rtx x, cond, insn;
{
 ...
 retry:
  ...
  switch (code)
    ...
    case CALL:
      /* If this call can throw and reach an external handler (in a
	 caller), we need to preserve the EH_USES registers.  Calls that
	 can reach a handler in this function (throw "internal") are
	 already accounted for by data-flow info.  */
      if (flag_exceptions && can_throw_external (insn))
	{
	  int i;
