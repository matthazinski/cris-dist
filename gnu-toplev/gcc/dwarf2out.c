...
/* Record call frame debugging information for an expression EXPR,
  ...
  BEGIN AXIS LOCAL
  Rule 15:     Saving the return-address in a register (presumably after
	       that register has been saved on stack).
  END AXIS LOCAL
...
/* BEGIN AXIS LOCAL
  Rule 15:
  (set <reg1> <reg2>:return_address)
  effects: flush queued register saves, mark reg1 as holding
	   return_address.
  END AXIS LOCAL.  */
  ...
  switch (GET_CODE (dest))
	...
	case REG:
	  if (cfa.reg == (unsigned) REGNO (src))
	    ...
	    ;
	  /* BEGIN AXIS LOCAL */
	  else if (REG_P (INCOMING_RETURN_ADDR_RTX)
		   && REGNO (INCOMING_RETURN_ADDR_RTX) == REGNO (src))
	    {
	      /* Rule 15: Saving the incoming return address in another
		 register.  That register should be a call-saved
		 register and thus should have been saved just before,
		 but it might have been flushed.  */
	      if (clobbers_queued_reg_save (expr))
		flush_queued_reg_saves ();
	      dwarf2out_return_reg (label, DWARF_FRAME_REGNUM (REGNO (dest)));
	      return;
	    }
	  /* END AXIS LOCAL */
	  else
