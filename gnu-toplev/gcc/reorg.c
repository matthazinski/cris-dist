...
find_end_label ()
{
  ...
  if (GET_CODE (insn) == BARRIER
	  ...
	  /* BEGIN AXIS LOCAL */
	  || (GET_CODE (PATTERN (PREV_INSN (insn))) == PARALLEL
	      && (GET_CODE (XVECEXP (PATTERN (PREV_INSN (insn)), 0, 0))
		  == RETURN))))
	  /* END AXIS LOCAL */
    {
     ...
     rtx insn;
{
  ...
  if (GET_CODE (trial) == JUMP_INSN
      && (simplejump_p (trial)
	  || GET_CODE (PATTERN (trial)) == RETURN
	  /* BEGIN AXIS LOCAL */
	  || (GET_CODE (PATTERN (trial)) == PARALLEL
	      && GET_CODE (XVECEXP (PATTERN (trial), 0, 0)) == RETURN)))
	  /* END AXIS LOCAL */
     ...
     rtx insn;
{
  ...
  if ((next_trial == next_active_insn (JUMP_LABEL (insn))
	  ...
	  && (simplejump_p (next_trial)
	      ...
	      /* BEGIN AXIS LOCAL */
	      || (GET_CODE (PATTERN (next_trial)) == PARALLEL
		  && GET_CODE (XVECEXP (PATTERN (next_trial), 0, 0)) == RETURN))))
	      /* END AXIS LOCAL */
    {
     ...
     int *pannul_p;
{
  ...
  if (! simplejump_p (XVECEXP (seq, 0, 0))
	  ...
	  /* BEGIN AXIS LOCAL */
	  && (GET_CODE (PATTERN (XVECEXP (seq, 0, 0))) != PARALLEL
	      || (GET_CODE (XVECEXP (PATTERN (XVECEXP (seq, 0, 0)), 0, 0))
		  != RETURN))))
	  /* END AXIS LOCAL */
    return delay_list;
  ...
  if (new_thread != thread)
	...
	abort ();
	  ...
	  && (simplejump_p (new_thread)
	      ...
	      /* BEGIN AXIS LOCAL */
	      || (GET_CODE (PATTERN (new_thread)) == PARALLEL
		  && (GET_CODE (XVECEXP (PATTERN (new_thread), 0, 0))
		      == RETURN)))
	      /* BEGIN AXIS LOCAL */
	  ...
	  && (target_label = JUMP_LABEL (insn)) != 0)
	{
	  ...
	  if (next && GET_CODE (next) == JUMP_INSN
		  ...
		  || (GET_CODE (PATTERN (next)) == PARALLEL
		      && GET_CODE (XVECEXP (PATTERN (next), 0, 0)) == RETURN))
		  /* END AXIS LOCAL */
      ...
      if (redundant_insn (XVECEXP (pat, 0, 1), delay_insn, 0))
	{
	 ...
	 smaller code.  */
	  ...
	  && (GET_CODE (PATTERN (delay_insn)) == RETURN
	      /* BEGIN AXIS LOCAL */
	      || (GET_CODE (PATTERN (delay_insn)) == PARALLEL
		  && GET_CODE (XVECEXP (PATTERN (delay_insn), 0, 0)) == RETURN))
	      /* END AXIS LOCAL */
	  && next
      ...
      if (target_label)
	{
	  ...
	  if (trial && GET_CODE (PATTERN (trial)) == SEQUENCE
		  ...
		  || (GET_CODE (PATTERN (XVECEXP (PATTERN (trial), 0, 0)))
		      == PARALLEL
		      && GET_CODE (XVECEXP (PATTERN (XVECEXP (PATTERN (trial), 0, 0)),
					    0, 0)) == RETURN))
		  /* END AXIS LOCAL */
  ...
  for (insn = first; insn; insn = NEXT_INSN (insn))
	...
	continue;
	 ...
	 insns for its delay slots, if it needs some.  */
	  ...
	  /* BEGIN AXIS LOCAL */
	  || (GET_CODE (PATTERN (jump_insn)) == PARALLEL
	      && GET_CODE (XVECEXP (PATTERN (jump_insn), 0, 0)) == RETURN))
	  /* END AXIS LOCAL */
	{
