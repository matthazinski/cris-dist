...
static boolean cris_elf_set_mach_from_flags
...
static boolean cris_elf_copy_private_bfd_data PARAMS ((bfd *, bfd *));
		...
		if (!elf_hash_table (info)->dynamic_sections_created
			...
			&& (h->elf_link_hash_flags & ELF_LINK_HASH_DEF_REGULAR)))
		  {
		    ...
		    /* This is actually a static link, or it is a
		       ...
		       local because of a version script, or we're not
...
/* Try to fold PLT entries with GOT entries.  There are two cases when we
   ...
   executable for calls, is a loss in performance.  Still, losing a
     ...
     struct elf_link_hash_entry *h;
{
  ...
  if (h->type == STT_FUNC
      ...
      if (info->shared && h->got.refcount > 0)
	{
	  ...
	  BFD_ASSERT ((s->_raw_size % plt_entry_size) == 0);
  ...
  for (rel = relocs; rel < rel_end; rel++)
	...
	case R_CRIS_32_GOTPLT:
	  if (dynobj == NULL)
		...
		{
		  (*_bfd_error_handler)
		    (_("v10/v32 compatible object %s must not contain a PIC relocation"),
	      ...
	      && imach != bfd_mach_cris_v10_v32))
	{
	  (*_bfd_error_handler)
	    ((imach == bfd_mach_cris_v32)
	     ? _("%s contains CRIS v32 code, incompatible with previous objects")
	     : _("%s contains non-CRIS-v32 code, incompatible with previous objects"),
	     bfd_archive_filename (ibfd));
	  bfd_set_error (bfd_error_bad_value);
	  return false;
	}
...
#define elf_backend_got_sym_visibility	STV_HIDDEN

/* Not actually used by core ELF bfd.  Make things go boom if this
   becomes used.  */
#define elf_backend_plt_header_size	((bfd_vma) -1)
