...
# We provide the padded address of the end of read-only code and data,
# smuggled in as an expression.  It will equal __Sdata if the
# read-only segment (assuming only one such segment) and the
# read-writable segment are consecutive (no address holes).
# Beware that in this binutils version, "-Tdata ..."
# (aka. "--section-start .data = ..." in this version only) doesn't
# set the start of the (read-writable data) *segment*, but only the
# .data *section*, which isn't (necessarily) first in that segment; it
# will break if .init_array or any of its friends are used.
# Beware also: this macro is evaluated as an expression:
# ". = DATA_ADDR ;"
DATA_ADDR='ALIGN(32); PROVIDE (__Etext = .)'
