...
extern const int cris_long_jump_size PARAMS ((void));

/* This will cause warnings from ISO C compilers (like gcc) in contexts
   where it's treated as a declaration.  */
#define md_long_jump_size cris_long_jump_size ()
...
/* Defining this corresponds to the MD_APPLY_SYM_VALUE "fix"
   in FSF CVS, where this macro is gone.  See PR gas/1049.  */
#define TC_FIX_ADJUSTABLE(X) tc_fix_adjustable(X)
